...
void Ball::Update(float dt)
{
	...
	glm::vec2 next_pos = pos + dir_speed;
	...
	for (uint32_t i = 0; i < entities.size(); ++i)
	{
		...
		glm::vec2 ld(entities[i]->pos.x - entities[i]->size.x / 2.f, entities[i]->pos.y - entities[i]->size.y / 2.f);
		glm::vec2 ru(entities[i]->pos.x + entities[i]->size.x / 2.f, entities[i]->pos.y + entities[i]->size.y / 2.f);
